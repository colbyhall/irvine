...
void D3D12FreeSlotDescriptorHeap::free(D3D12_CPU_DESCRIPTOR_HANDLE handle) const {
	...
	ASSERT(m_free_slots[index].load());
...
D3D12BindlessHandle D3D12BindlessDescriptorHeap::alloc_texture2d() const {
	while (true) {
...
void D3D12BindlessDescriptorHeap::free_texture2d(D3D12BindlessHandle handle) const {
	const D3D12_CPU_DESCRIPTOR_HANDLE start = m_heap->GetCPUDescriptorHandleForHeapStart();
...
D3D12Context::D3D12Context() {
	// Load d3d12 dynamically to not bloat our binary as we could use other backends
	d3d12_library = Library::open("d3d12.dll").unwrap();
	create_device = (FnCreateDevice)d3d12_library.find("D3D12CreateDevice");
	serialize_root_signature = (FnSerializeRootSignature)d3d12_library.find("D3D12SerializeRootSignature");
	get_debug_interface = (FnGetDebugInterface)d3d12_library.find("D3D12GetDebugInterface");
